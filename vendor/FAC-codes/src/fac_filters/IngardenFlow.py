...
def ingarden_flow(image: FloatArray, beta: float, dt: float, iterations: int) -> FloatArray:
    ...
    for _ in range(iterations):
        ...
        for i in range(m):
            for j in range(n):
                ...
                bg = np.array([[ix[i, j]], [iy[i, j]]], dtype=np.float64)
                bgk1 = np.array([[ixx[i, j]], [ixy[i, j]]], dtype=np.float64)
                bgk2 = np.array([[ixy[i, j]], [iyy[i, j]]], dtype=np.float64)
                bg1 = scalar(bg[0, 0])
                bg2 = scalar(bg[1, 0])
                ...
                phinv12 = p * ginv[0, 1] + q * v1 * v2 + r * (
                    scalar(ginv[0, :] @ bg) * v2 + scalar(g[1, :] @ bg) * v1
                )
...
def main() -> None:
    parser = build_metric_parser(
        'Apply the Ingarden flow to a selected image.',
        DEFAULT_BETA,
        DEFAULT_DT,
        DEFAULT_ITERATIONS,
    )
    ...
    start = perf_counter()
    result = ingarden_flow(image, args.beta, args.dt, args.iterations)
    elapsed = perf_counter() - start

    output_path = args.output if args.output is not None else default_output_path(image_path, 'ingarden')
