...
The observational unit is one prespecified axial slice per imaging study.
TCGA-LGG and UCSF-PDGM contribute one study per patient. The 150 BraTS studies
represent 143 unique patients; repeated studies remain in the same
resampling cluster. Means include zero-Dice cases. Percentile bootstrap $95\%$
confidence intervals use 20,000 resamples. Case-level differences are resampled
for paired comparisons, whereas boundary analyses use patient-clustered
resampling. The corresponding fixed random seeds are 20260715 for primary paired
analyses and 20260723 for cross-cohort boundary analyses.

Spearman correlations quantify the association between boundary descriptors
and candidate ceilings; partial
Spearman analyses controlling for area, compactness and the normalized
three-pixel ring intensity separation defined above use 5,000
clustered resamples. The UCSF grade comparison is exploratory and uses a
...
comparisons are treated as developmental rather than confirmatory.
