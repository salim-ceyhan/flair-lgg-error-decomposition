...
def _resolve_facseg_root():
    ...
    candidates.extend((_REPO_ROOT / "facseg", _REPO_ROOT.parent / "facseg"))
...
if _FSG_PATH is not None:
    _FSG = str(_FSG_PATH)
    NEWMETRIC_BACKEND = "theory-aligned-local"
    from src.newmetric_corrected import NewMetric
    sys.path.insert(0, str(_FSG_PATH / "src"))
    from facseg.roi_utils import extract_roi
else:
    raise ImportError(
        "The bundled facseg utilities were not found under facseg/src/facseg. "
...
elif os.path.isdir(os.path.join(_PROJ, "data", "tcga_lgg_dataset")):
    # Prefer the repository cohort when it is present. A stale local cache may
    # contain a differently materialized cohort under the same case names.
