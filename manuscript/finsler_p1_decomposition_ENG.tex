...
tumour reached by any single candidate. Boundary availability is summarized by
the pool-oracle Dice $D_{\max}=\max_jD(M_j,Y)\in[0,1]$, using the Dice definition
given above. The selection gap is
$\Delta_{\mathrm{sel}}=D_{\max}-D(\widehat M,Y)\ge0$. Errors are assigned hierarchically:
$R_{\max}<0.50$ indicates localization failure;
$R_{\max}\ge0.50$ and $D_{\max}<0.70$ indicates boundary-representation failure;
$D_{\max}\ge0.70$ with $\Delta_{\mathrm{sel}}\ge0.10$ indicates selection
failure; all remaining cases are classified as near-ceiling/successful. These
...
retrospective class labels, not the predicted masks.
...
Although the 27-setting sensitivity grid preserves the qualitative dominance of
selection failures, the exact class counts change with these definitions,
