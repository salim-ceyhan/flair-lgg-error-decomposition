...
\begin{itemize}[leftmargin=1.5em,itemsep=0.35em,topsep=0.25em]
  \item Errors were separated into localization, representation, and selection.
  \item Expanded candidates raised oracle Dice from $0.872$ to $0.906$.
  \item The canonical pool covered $103/110$ cases at Dice$\geq0.70$.
  \item Weak boundaries predicted lower candidate ceilings across three cohorts.
  \item Frozen single-FLAIR segmentation achieved $0.712$ Dice on UCSF-PDGM.
\end{itemize}
\end{tcolorbox}
...
candidate. We separated these error sources using a fixed method without learned
...
$\alpha$-cut candidates were added only for retrospective capacity analysis.
The whole-tumour target was evaluated on the ground-truth-selected
...
delivered Dice was $0.789$ and the candidate-pool ceiling was $0.872$.
Localization succeeded in $106/110$ cases; $103/110$ contained a candidate with
Dice$\geq0.70$, while $16/103$ representable cases retained a selection gap of
at least $0.10$. Nested seed and $\alpha$-cut expansion raised the retrospective
ceiling to $0.906$ and satisfied the prespecified $0.005$ practical-saturation
margin, without changing delivered Dice. Weak-boundary fraction was negatively
...
covariate adjustment. The frozen method achieved $0.712$ mean Dice on UCSF-PDGM
...
they do not replace the primary delivered mask. Reference masks were also used
...
The two adoption thresholds were developmental choices. Their sensitivity was
...
were examined retrospectively using the upper quartiles of reference-mask area
and normalized reference-centroid eccentricity; these quantities were not inputs
to the gate.
...
Computational cost was measured over all 110 TCGA slices on an Intel Core
Ultra~7 265K CPU (20 physical cores, 31.6~GiB RAM, Windows~11). The full canonical timing included
NewMetric preprocessing, candidate generation and label-free selection. The
expanded-budget timing was recorded separately and measures preprocessing plus
...
batch run was not stored in its provenance, whereas the expanded-budget audit
was run serially; the reported values are therefore descriptive computational
...
Descriptive sensitivity was assessed over the complete $3\times3\times3$ grid
$R_{\max}\in\{0.40,0.50,0.60\}$, $D_{\max}\in\{0.60,0.70,0.80\}$, and
selection-gap threshold $\in\{0.05,0.10,0.15\}$. This audit changed only the
...
Spearman correlations quantified the association between boundary descriptors
...
The 27-setting threshold audit yielded 3--5 localization failures, 2--11
boundary-representation failures, 12--19 selection failures, and 81--87
near-ceiling cases. Canonical counts were unchanged when the oracle threshold
was varied from $0.60$ to $0.70$ and when the selection-gap threshold was varied
from $0.10$ to $0.15$; the largest redistribution occurred at the stricter
oracle threshold of $0.80$. Selection failures outnumbered localization and
boundary failures throughout the grid, although the exact counts remained
...
The full canonical method required a mean of $1.659$~s per slice (median
...
$0.60$ did not alter adoption because every eligible $M_1$ had
$f_{\mathrm{in}}(M_1)=1$. Raising the first-mask threshold from $0.55$--$0.60$
to $0.65$, $0.70$, and $0.75$ increased adoptions from 4 to 5, 7, and 9, while
deteriorations increased from 0 to 1, 3, and 4. The frozen $0.65$ value is thus a
developmental operating point rather than a theoretically unique constant. No
upper-quartile large tumour triggered the frozen gate; one upper-quartile
peripheral tumour triggered it and improved. These counts are too small to
establish anatomical safety, and the single deterioration among the five frozen
adoptions was substantial (Dice change $-0.312$).
