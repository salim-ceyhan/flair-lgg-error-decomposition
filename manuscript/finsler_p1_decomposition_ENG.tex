...
The evaluated subset comprises WHO grade 2 ($n{=}56$) and grade 3 ($n{=}43$)
...
Let $I_{\mathrm{raw}}$ denote the prespecified FLAIR slice. Intensities are
...
$\beta_{\mathrm{NM}}=5.0$, $\Delta t=0.15$, and three iterations. Its result is
...
The Sobel gradient magnitude $G=\lVert\nabla F_n\rVert$ is converted to an edge
...
$p_{30}(I_n)\le I_n\le p_h(I_n)$. At most ten seeds are retained per window,
...
\end{equation}
Evolution terminates when the component exceeds $25\%$ of $B$, exhibits a
fourfold area jump, satisfies the predefined acceleration condition, or meets the
edge-wall condition. Successive components whose relative area change does not
exceed $0.06$ form one stable interval. Candidate recording begins after the
component reaches $\max(40,0.02|B|)$ pixels; the duration of its stable interval
defines persistence $\pi(M)$. Thus, seeds are determined from a FLAIR-restricted
...
brain ROI is therefore estimated from the image, with peripheral bright tissue
removed and orbital regions excluded when their dark-centred, approximately
circular and lateral configuration satisfies the predefined anatomical rules.
...
$c$-means~\cite{bezdek1981} is applied to intra-support intensities. The darkest class is
excluded, and connected components of fixed membership $\alpha$-cuts are added
...
$a\in\{0,6,11,15,23,39\}$ are evaluated without removing candidates admitted
...
cases, and the numbers exceeding Dice thresholds of 0.50 and 0.70 are secondary
measures. Boundary agreement is characterized by average symmetric surface
distance, 95th-percentile Hausdorff distance, and boundary F1 within two pixels;
distance measures are summarized only when the two masks overlap
~\cite{taha2015}. Lesion-level
sensitivity, precision and F1 use connected-component matching at
$\operatorname{IoU}\ge0.10$.

Boundary strength is measured in inner and outer bands of the reference contour.
The primary descriptor is the fraction of boundary locations whose NewMetric
gradient magnitude fell below the intra-support median. Additional descriptors
include normal-direction gradient magnitude and sign consistency, inner--outer
ring intensity separation, tumour area and compactness. These quantities use
...
Spearman analyses controlling for area, compactness and ring contrast use 5,000
