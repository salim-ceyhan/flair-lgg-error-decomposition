...
slice, $|A|$ the number of pixels in a set $A\subseteq\Omega$, and
$\epsilon=10^{-8}$ a numerical safeguard against division by zero. Intensities are
...
$\operatorname{CC}_s(A)$ is the 8-connected component of binary set $A$ that
...
$n_{\min}=\max(40,0.02|B|)$ pixels denote the minimum candidate area.
...
A stable interval is a maximal consecutive threshold interval $[t_-,t_+]$
over which $a_j\le1.06a_{j_+}$, where $a_{j_+}$ is the component area at the
high-threshold end. Its mask $M$ is recorded only after reaching $n_{\min}$, and
its persistence is the score-threshold width
$\pi(M)=t_+-t_-\ge0$. Thus, seeds are determined from a FLAIR-restricted
score maximum, whereas candidate boundaries are determined by the topology of
score superlevel sets.

For every nonempty candidate mask $M\subseteq B$, label-free quality combines area, mean evidence,
compactness and solidity:
\begin{equation}
 Q(M)=|M|\,\overline E_M\,
 \frac{4\pi|M|}{\operatorname{per}(M)^2}\,
 \frac{|M|}{|\operatorname{hull}(M)|}.
\label{eq:quality-primary}
\end{equation}
Here $\overline E_M=|M|^{-1}\sum_{x\in M}E(x)$ is mean evidence,
$\operatorname{per}(M)$ is the 8-connected digital perimeter in pixels, and
$\operatorname{hull}(M)$ is the binary convex hull of $M$. Consequently,
$4\pi|M|/\operatorname{per}(M)^2$ is compactness,
$|M|/|\operatorname{hull}(M)|$ is solidity,
and $Q(M)\ge0$ is the resulting label-free quality score.
...
interval; it does not, by itself, establish tumour specificity. The exponent is
...
excluded, and 8-connected components of the binary membership sets
...
sensitivity, precision and F1 use 8-connected predicted and reference components,
...
$4\pi|Y|/\operatorname{per}(Y)^2$. These quantities use ground truth only for retrospective error
attribution.
...
assumed: nearby exponents perform similarly, whereas excessive persistence
weighting degrades accuracy.
