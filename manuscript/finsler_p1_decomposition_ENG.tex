...
\begin{itemize}[leftmargin=1.5em,itemsep=0.35em,topsep=0.25em]
  ...
  \item The canonical pool covered $103/110$ at Dice$\geq0.70$; $16/103$ retained gaps $\geq0.10$.
  \item Weak tumour boundaries correlated with lower candidate ceilings across three cohorts.
  \item Frozen single-FLAIR fixed-slice segmentation achieved $0.712$ Dice on external UCSF-PDGM.
\end{itemize}
\end{tcolorbox}
...
We separated these error sources using a primary method with no learned weights
that combines edge-preserving direction-dependent diffusion, score-topology
threshold sweeping, and a closed-form label-free selection criterion; fuzzy
$\alpha$-cut candidates were added only in a retrospective capacity analysis.
The whole-tumour target was evaluated on the
...
truth was used for slice selection, fixed-parameter development on TCGA-LGG,
and retrospective evaluation, including candidate-pool ceilings and error
decomposition; it did not enter case-level candidate generation or selection
after the method was fixed. In TCGA-LGG, the label-free method achieved a mean
...
marginal gains satisfying a prespecified $0.005$ practical-saturation margin;
