...
def improved_normalized_miron_flow(
    image: FloatArray,
    beta: float,
    dt: float,
    iterations: int,
    ...
    for _ in range(iterations):
        ...
        for i in range(m):
            for j in range(n):
                ...
                c = (1.0 - 1.0 / detg[i, j]) * (1.0 / v_quad)
                ck1 = (1.0 / v_quad) * ((1.0 / (detg[i, j] ** 2)) * detgk1[i, j] - c * vk1)
                ck2 = (1.0 / v_quad) * ((1.0 / (detg[i, j] ** 2)) * detgk2[i, j] - c * vk2)
...
def main() -> None:
    parser = build_metric_parser(
        'Apply the Improved Normalized Miron flow to a selected image.',
        DEFAULT_BETA,
        DEFAULT_DT,
        DEFAULT_ITERATIONS,
    )
    ...
    start = perf_counter()
    result = improved_normalized_miron_flow(image, args.beta, args.dt, args.iterations)
    elapsed = perf_counter() - start

    output_path = args.output if args.output is not None else default_output_path(image_path, 'inmflow')
