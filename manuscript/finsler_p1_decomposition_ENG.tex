...
with a minimum separation of 12 pixels. For each seed $s$, the threshold is
decreased in 200 equally spaced levels from $S(s)$ to $0.20S(s)$, and the
connected component containing that seed is followed:
\begin{equation}
 C_t(s)=\operatorname{CC}_{s}\big(\{S\ge t\}\cap B\big).
\label{eq:seeded-components}
\end{equation}
Here $t$ is the descending score threshold,
$\operatorname{CC}_s(A)$ is the 4-connected component of binary set $A$ that
contains $s$, and $C_t(s)$ is its mask at threshold $t$. Let
$a_j=|C_{t_j}(s)|$ and $\Delta_j=a_j-a_{j-1}$ at successive thresholds
$t_1>\cdots>t_{200}$, and let
...
Plateau tracking begins once $a_j\ge30$ pixels. Its representative mask $M$ is
the largest component reached within the run, with holes filled only when hole
filling does not increase its area beyond $2|M|$. The persistence attribute is
the dimensionless run length
$\pi(M)=m\in\{1,\ldots,200\}$, not the numerical width of the score-threshold
interval. Thus, seeds are determined from a FLAIR-restricted
score maximum, whereas candidate boundaries are determined by the topology of
